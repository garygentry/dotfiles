# Documentation

Welcome to the dotfiles management system documentation.

## Table of Contents

### Getting Started
- [Installation](installation.md) - How to install and bootstrap the system
- [Quick Start](quick-start.md) - Get up and running in 5 minutes
- [Configuration](configuration.md) - Configure the system for your needs

### Core Concepts
- [Architecture](architecture.md) - System design and component overview
- [Modules](modules.md) - Understanding the module system
- [Profiles](profiles.md) - Working with profiles
- [Dependencies](dependencies.md) - How module dependencies work

### Development
- [Creating Modules](creating-modules.md) - Step-by-step guide to creating new modules
- [Module Reference](module-reference.md) - Complete module.yml schema reference
- [Shell Helpers](shell-helpers.md) - Available helper functions for module scripts
- [Templates](templates.md) - Using Go templates in your modules

### Advanced Topics
- [Secrets Management](secrets-management.md) - Working with 1Password and secrets
- [State Management](state-management.md) - How installation state is tracked
- [CI/CD Integration](ci-cd.md) - Using dotfiles in automation

### Reference
- [CLI Commands](cli-reference.md) - Complete command-line interface reference
- [Environment Variables](environment-variables.md) - Available environment variables
- [Troubleshooting](troubleshooting.md) - Common issues and solutions
- [Contributing](../CONTRIBUTING.md) - How to contribute to the project
